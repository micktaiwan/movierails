...
class Proposer
  ...
  attr_writer :use_items_weight
	def initialize
		@db = [] 
		@user_item = []
		@users = []
		@items = []
    @use_items_weight = nil
	...
	def propose(user)
		...
		@items = items.sort{|x,y| - x[1]<=>y[1]}	
		@users = users.sort{|x,y| - x[1]<=>y[1]}
...
if __FILE__ == $0
  ...
  # or
  #p.use_items_weight = true
  #items_users = [[1,'A',2],[1,'B',1],[1,'C',5],[2,'A',5],[2,'B',3],[2,'E',3],[3,'A',2],[3,'D',1],[4,'C',5]] 
	items_users = []
	100.times { |i|
		a = [rand(10),(rand(26)+65).chr,rand(5)]
